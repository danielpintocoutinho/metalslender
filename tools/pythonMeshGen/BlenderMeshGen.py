...
class MyApp():
      ...
      if isinstance(egg, EggPolygon):
        ...
        if type == "Full":
            ...
            if (self.firstNode == None):
                ...
                if node.vertex[self.lowestVertex].getX() < self.firstNode.vertex[self.lowestVertex].getX() and node.vertex[self.lowestVertex].getY() < self.firstNode.vertex[self.lowestVertex].getY():
                    self.firstNode = node
        ...
        for r in range(int(sqrt(self.nodeCount))): 
          for c in range(int(sqrt(self.nodeCount))):
            ...
            if node == None:
                ...
                # Width
                file.write(str(node.vertex[0].getY() - node.vertex[3].getY()) + ',' )
                # Height
                file.write('0' + ',' )
                # PosX
                file.write(str((node.vertex[0].getX() + node.vertex[1].getX())/2) + ',')
                # PosY
                file.write(str((node.vertex[0].getY() + node.vertex[3].getY())/2) + ',')
                # PosZ
                file.write('0')
                ...
                for i in range(8):
                    ...
                    if nnode == None:
                        ...
                        # Width
                        file.write(str(nnode.vertex[0].getY() - nnode.vertex[3].getY()) + ',' )
                        # Height
                        file.write('0' + ',' )
                        # PosX
                        file.write(str((nnode.vertex[0].getX() + nnode.vertex[1].getX())/2) + ',')
                        # PosY
                        file.write(str((nnode.vertex[0].getY() + nnode.vertex[3].getY())/2) + ',')
                        # PosZ
                        file.write('0')                    
    ...
    def setNeighbors(self, node, row, col):
        if not(node == None):
            ...
            # left top 
            if (col-1)>=0 and (row+1)<sqrt(self.nodeCount):
    ...
    def analyzeVertex(self, node):
        ...
        for i in range(4):
            if node.vertex[i].getX() < node.vertex[self.lowestVertex].getX() and node.vertex[i].getY() < node.vertex[self.lowestVertex].getY():
                self.lowestVertex = i
        ...
        for i in range(4):
            if not(i==self.lowestVertex):
                ...
                z1 = float(int(node.vertex[self.lowestVertex].getY()*precision)/precision)
                z2 = float(int(node.vertex[i].getY()*precision)/precision)
